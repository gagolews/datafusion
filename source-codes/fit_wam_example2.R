...
m <- 100
realw <- rbeta(n, 1, 5)
realw <- realw/sum(realw) # real weights follow a beta distribution
...
p <- seq(-1, 10, length=101)
bp <- sapply(p, function(p) {
   # res <- replicate(1, {
      w <- fit_wam_L2_regular(X, Y, p)
      error_wam_test(w)
   # })
   # res[,which.min(res["L2",])]
})
# cairo_pdf("~/Publications/Papers/02-InProgress/IPMU2016_fitagop/wam_regularization_L2.pdf", height=4, width=6)
par(mar=c(4, 4, 0.5, 0.5))
matplot(p, bp[2,], type='l', log="y", xlab=expression(lambda), ylab="test sample error", las=1,)
